...
public class NoteEditor extends Activity {
    ...
    private static final String[] PROJECTION =
            new String[] {
                    ...
                    NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR  // 新增：背景颜色字段
            };

    // 背景颜色索引
    private static final int COLUMN_INDEX_BACKGROUND_COLOR = 3;

    // 背景颜色数组
    private static final int[] BACKGROUND_COLORS = {
            NotePad.Notes.COLOR_WHITE,   // 白色
            NotePad.Notes.COLOR_BLUE,    // 浅蓝色
            NotePad.Notes.COLOR_GREEN,   // 浅绿色
            NotePad.Notes.COLOR_YELLOW,  // 浅黄色
            NotePad.Notes.COLOR_PINK,    // 浅粉色
            NotePad.Notes.COLOR_GRAY     // 浅灰色
    };

    // 背景颜色菜单项ID数组
    private static final int[] COLOR_MENU_IDS = {
            R.id.color_white,
            R.id.color_blue,
            R.id.color_green,
            R.id.color_yellow,
    ...
    private int mCurrentBackgroundColor = NotePad.Notes.DEFAULT_BACKGROUND_COLOR;
    ...
    protected void onResume() {
        ...
        if (mCursor != null) {
            ...
            // 加载背景颜色
            if (mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR) != -1) {
                mCurrentBackgroundColor = mCursor.getInt(COLUMN_INDEX_BACKGROUND_COLOR);
                applyBackgroundColor(mCurrentBackgroundColor);
            }
    ...
    private void applyBackgroundColor(int color) {
        ...
        // 同时设置EditText的背景色，但保持一定的透明度以便阅读
        if (mText != null) {
            int textBackgroundColor = (color & 0x00FFFFFF) | 0xE0000000; // 添加透明度
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        // 添加背景颜色子菜单
        SubMenu backgroundSubMenu = menu.addSubMenu("背景颜色");
        inflater.inflate(R.menu.background_colors_menu, backgroundSubMenu);
     ...
     * 更新颜色菜单的选中状态
     */
    private void updateColorMenuSelection(Menu menu) {
        for (int i = 0; i < BACKGROUND_COLORS.length; i++) {
            MenuItem item = menu.findItem(COLOR_MENU_IDS[i]);
            if (item != null) {
                item.setChecked(mCurrentBackgroundColor == BACKGROUND_COLORS[i]);
            }
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if(id== R.id.menu_save) {
            ...
            changeBackgroundColor(NotePad.Notes.COLOR_WHITE);
            return true;
        } else if (id == R.id.color_blue) {
            changeBackgroundColor(NotePad.Notes.COLOR_BLUE);
            return true;
        } else if (id == R.id.color_green) {
            changeBackgroundColor(NotePad.Notes.COLOR_GREEN);
            return true;
        } else if (id == R.id.color_yellow) {
            changeBackgroundColor(NotePad.Notes.COLOR_YELLOW);
            return true;
        } else if (id == R.id.color_pink) {
            changeBackgroundColor(NotePad.Notes.COLOR_PINK);
            return true;
        } else if (id == R.id.color_gray) {
            changeBackgroundColor(NotePad.Notes.COLOR_GRAY);
            return true;
        }
    ...
    private void changeBackgroundColor(int color) {
        ...
        // 立即保存颜色设置
        updateNoteBackgroundColor(color);
        Toast.makeText(this, "背景颜色已更改", Toast.LENGTH_SHORT).show();
    }

    /**
     * 更新笔记的背景颜色
     */
    private void updateNoteBackgroundColor(int color) {
...
//END_INCLUDE(paste)
    ...
    private final void updateNote(String text, String title) {
        ...
        ContentValues values = new ContentValues();
        values.put(NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE, System.currentTimeMillis());
        values.put(NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR, mCurrentBackgroundColor);
